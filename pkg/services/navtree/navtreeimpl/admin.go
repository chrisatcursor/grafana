...
func (s *ServiceImpl) getAdminNode(c *contextmodel.ReqContext) (*navtree.NavLink, error) {
	...
	if c.HasRole(identity.RoleAdmin) &&
		featuremgmt.OpenFeatureIsEnabledGlobally(s.features, featuremgmt.FlagProvisioning) {
		generalNodeLinks = append(generalNodeLinks, &navtree.NavLink{
			Text:     "Provisioning",
			Id:       "provisioning",
	...
	if s.license.FeatureEnabled("groupsync") &&
		featuremgmt.OpenFeatureIsEnabled(ctx, s.features, featuremgmt.FlagGroupAttributeSync) &&
