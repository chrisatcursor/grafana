...
function formatValue(toggle: FeatureFlagDTO): string {
  if (toggle.valueType === 'boolean') {
    return toggle.enabled ? 'true' : 'false';
  }

  if (toggle.value === undefined || toggle.value === null) {
    return '-';
  }

  if (typeof toggle.value === 'object') {
    return JSON.stringify(toggle.value);
  }
      ...
      <table className="filter-table">
        <thead>
          <tr>
            ...
            <th>
              <Trans i18nKey="admin.feature-toggles.column-value-type">Type</Trans>
            </th>
            <th>
              <Trans i18nKey="admin.feature-toggles.column-value">Value</Trans>
            </th>
            <th>
        ...
        <tbody>
          {featureToggles.map((toggle) => (
            <tr key={toggle.name}>
              ...
              <td>
                <Text color="secondary">{toggle.valueType}</Text>
              </td>
              <td className={styles.valueCell}>
                <Text color="secondary">{formatValue(toggle)}</Text>
              </td>
